...
struct HyleLoop {
    ...
    float extra;        // node: radius; pong: 0 => toward A (warm), 1 => toward B (cool)
    float2 position;
    float2 dir;
};
...
fragment float4 hyleLoopFragmentShader(HyleLoopPacket in [[stage_in]]) {
    ...
    // pong: bright body, colored by stream direction
    float4 color = in.extra < 0.5
        ? float4(1.0, 0.70, 0.28, 1.0)                      // toward A: warm
        : float4(0.37, 0.83, 0.95, 1.0);                    // toward B: cool
