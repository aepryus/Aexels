...
class PathOfDiscovery {
    ...
    static func glyphs(s: CGFloat) -> [GlyphView] {
        ...
        let escapingGlyph: ArticleGlyph = ArticleGlyph(article: escaping, radius: 102*s+2*p, x: 200*s, y: 270*s)
        let covarianceGlyph: ArticleGlyph = ArticleGlyph(article: covariance, radius: 124*s+2*p, x: 230*s, y: 410*s)
        let reciprocityGlyph: ArticleGlyph = ArticleGlyph(article: reciprocity, radius: 128*s+2*p, x: 70*s, y: 580*s)
        let eeemceeGlyph: ArticleGlyph = ArticleGlyph(article: eeemcee, radius: 132*s+2*p, x: 150*s, y: 730*s)
        let cupolaGlyph: ArticleGlyph = ArticleGlyph(article: cupola, radius: 92*s+2*p, x: 70*s, y: 930*s)
        let squishGlyph: ArticleGlyph = ArticleGlyph(article: squish, radius: 88*s+2*p, x: 270*s, y: 950*s)
